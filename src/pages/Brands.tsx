...
const Brands = () => {
  ...
  const [editBrand] = usePutBrandMutation();
  ...
  const UpdateModal = (brand: Inputs) => {
    setName(brand.name);
    setIdx(brand.id);
    setEditFile(brand.img);
            ...
            <div className="flex justify-end w-full pb-4">
              <button
                onClick={async () => {
                  ...
                  if (!name) return alert('Please write brand name');
                  const formData = new FormData();
                  formData.append('file', file);
          ...
          data.map((item: { img: string; name: string; id: number }) => {
            return (
              <Card className="dark:bg-graydark min-w-[140px]">
                <CardHeader
                  floated={false}
                  className="max-h-[50vh] md:h-[20vh] sm:h-[30vh] flex items-center justify-center px-1"
                >
                  <img
