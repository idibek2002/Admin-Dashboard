...
const Category = () => {
  ...
  return (
    <>
      ...
      <div className="max-w-full p-3 grid grid-cols-1 xl:grid-cols-6 lg:grid-cols-5 md:grid-cols-4 sm:grid-cols-3 gap-5 rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        {data.length > 0 &&
          data.map((item: { img: string; name: string; id: number }) => {
            return (
              <Card className="dark:bg-graydark min-w-[140px]">
                <CardHeader
                  floated={false}
                  className="max-h-[50vh] md:h-[20vh] sm:h-[30vh] flex items-center justify-center"
                >
                  <img
                ...
                <CardBody className="text-center py-2">
                  <Typography
                    variant="h4"
                    color="blue-gray"
                    className="dark:text-white text-[15px]"
                  >
                    {item.name}
                  </Typography>
                </CardBody>
                <CardFooter className="flex justify-center py-0 gap-7">
...
<Dialog
      ...
      >
        <DialogHeader className="flex items-center justify-between">
          <h1 className="text-[20px] dark:text-whiten">Update Category</h1>
          <button
            onClick={() =>{
              dispatch(handleChangeModal({ name: 'editModal', value: false }))
              resetForm()
